...
const settings = {
  ...
  'ComfyUI 显存与缓存': 'ComfyUI VRAM and cache',
  '默认服务器': 'Default server',
  '未命名服务端': 'Unnamed server',
  '显存状态服务端': 'VRAM status server',
  '刷新': 'Refresh',
  '读取显存状态失败': 'Failed to read VRAM status',
  '正在读取显存状态…': 'Reading VRAM status…',
  '显存已用 {used} / {total}，设备空闲 {free}': 'VRAM used {used} / {total}; device free {free}',
  'PyTorch 报告空闲 {free}': 'PyTorch reports {free} free',
  '卸载模型': 'Unload models',
  '完全释放': 'Release all',
  '已请求完全释放 ComfyUI 资源': 'Requested a full ComfyUI resource release',
  '已请求卸载 ComfyUI 模型': 'Requested ComfyUI model unload',
  '释放 ComfyUI 资源失败': 'Failed to release ComfyUI resources',
  '释放操作作用于整台 ComfyUI 服务；任务运行中时会在服务空闲后执行。完全释放会同时清空执行缓存，下次生成需要重新加载模型。':
    'Release affects the entire ComfyUI server. If a task is running, it takes effect once the server is idle. A full release also clears the execution cache, so models must reload for the next generation.',
  '本地任务结束后': 'After a local task finishes',
  '保留智能缓存（推荐）': 'Keep smart cache (recommended)',
  '队列空闲时卸载模型': 'Unload models when the queue is idle',
  '队列空闲时完全释放': 'Release all when the queue is idle',
  '自动策略只对 localhost、127.0.0.1 或 ::1 生效，不会自动清理远程或共享服务器。':
    'Automatic policies apply only to localhost, 127.0.0.1, or ::1. Remote and shared servers are never cleared automatically.',
