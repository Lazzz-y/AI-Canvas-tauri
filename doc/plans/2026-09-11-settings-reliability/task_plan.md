...
当前阶段：三个已确认阶段均已完成，并补充修复配置列表删除被同列表并发保存误判冲突的问题。原生锁、条件读写、同步落盘、删除收敛和同类型工具栏冲突已通过定向回归；真实桌面退出重开与断电验收仍未进行。
...
2. 普通设置按本次实际修改字段提交；providers 按连接和字段分解，工具栏按节点类型处理。数组默认作为对应字段的完整值；仅当带稳定 `id` 的对象列表或唯一字符串/数字列表发生纯删除时，按删除目标合并，以保留同列表同时发生的其他条目编辑与新增。保存携带读取基线，事务内比较当前值；其他同字段冲突仍拒绝并提示重新加载，不以最后写入者无条件获胜。
...
- 多实例修改不同字段可保留双方变更；配置列表的纯删除保留同期其他条目编辑与新增，其他同字段修改继续受冲突保护。旧版或不同存储源客户端不参与新协议时不声称拥有同等保证。
